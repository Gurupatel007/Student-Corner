...
const db = mysql.createConnection({
  ...
  if (err) {
    ...
    let hackathonsSQL = 'REPLACE INTO hackathons (id, deadlineDate, eventName, logo, location, month, day) VALUES\n';
    let detailsSQL = 'REPLACE INTO hackathon_details (eventid, hackathon_id, img, startingTime, endingTime, description, age, country) VALUES\n';

    hackathons.forEach((hackathon, index) => {
      hackathonsSQL += `(${hackathon.id}, '${hackathon.deadlineDate}', '${hackathon.eventName.replace(/'/g, "''")}', '${hackathon.logo}', '${hackathon.location.replace(/'/g, "''")}', '${hackathon.month}', '${hackathon.day}')`;

      detailsSQL += `('${hackathon.moreDetails.eventid}', ${hackathon.id}, '${hackathon.moreDetails.img}', '${hackathon.moreDetails.startingTime}', '${hackathon.moreDetails.endingTime}', '${hackathon.moreDetails.description.replace(/'/g, "''")}', '${hackathon.moreDetails.age}', '${hackathon.moreDetails.country.replace(/'/g, "''")}')`;
    ...
    db.query(hackathonsSQL, (err, result) => {
      ...
      db.query(detailsSQL, (err, result) => {
        if (err) throw err;
        console.log("Hackathon details data inserted");
